...
def HOG_cell_histogram(cell_direction, cell_magnitude, hist_bins):
    ...
    for row_idx in range(cell_size):
        for col_idx in range(cell_size):
            ...
            HOG_cell_hist[first_bin_idx] += (numpy.abs(curr_direction - first_bin_value)/(180.0/hist_bins.size)) * curr_magnitude
            HOG_cell_hist[second_bin_idx] += (numpy.abs(curr_direction - second_bin_value)/(180.0/hist_bins.size)) * curr_magnitude
